...
Parametertuner::Parametertuner(ros::NodeHandle nh_)
    :m_useJoystick(false)
{
    this->nh_ = nh_;
    m_ui.setupUi(this);
...
//	m_sub_paramList = this->nh_.subscribe("/config_server/parameter_list", 1, &Parametertuner::handleList, this);
//	m_sub_joystick = this->nh_.subscribe("/joy", 1, &Parametertuner::handleJoystickInput, this);
...
//    std::cout << "shit the fucking monkeu\n";
//	context.addWidget(w);
//}
...
QTreeWidgetItem* Parametertuner::insertParameterNode(const config_server::ParameterDescription& description, QTreeWidgetItem* ancestor)
{
    ...
    if (type == "int")
        newWidget = new IntParameterWidget(this->nh_, description);
    else if (type == "string")
        newWidget = new StringParameterWidget(this->nh_, description);
    else if (type == "float")
        newWidget = new FloatParameterWidget(this->nh_, description);
    else if (type == "bool")
        newWidget = new BoolParameterWidget(this->nh_, description);
