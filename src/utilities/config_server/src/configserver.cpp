...
//MK	std::ifstream stream(fname.c_str());
//MK	YAML::Parser p(stream);
	YAML::Node n = YAML::LoadFile(fname.c_str()); //MK

	if( n == NULL)
	{
		ROS_ERROR_STREAM("Could not read config file '" << fname << "'");
//MK	FIXME	if(stream.fail())
//MK	FIXME		ROS_ERROR("The I/O error might have been: %s", strerror(errno));
		return false;
	}
...
void ConfigServer::insertFromYAML(const YAML::Node& n, const std::string& path)
{
	if(n.Type() == YAML::NodeType::Map)
	{
		...
		{
			insertFromYAML(it->second, path + "/" + it->first.as<std::string>()); // FIXME 
		}
	}
	...
	if(n.Type() == YAML::NodeType::Scalar)
	{
		...
		p.desc.name = path;
		p.value = n.as<std::string>(); //FIXME FIXME 
		p.notify();
	}
}
